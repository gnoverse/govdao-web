...
export class AdenaService {
  ...
  // Adds a new custom network to Adena
  static async addNetwork(
    ...
    const response: IAdenaResponse = await adena.AddNetwork({
      chainId: chainID,
      rpcUrl: rpc,
      chainName: name
    });
    ...
    if (
      response.status === EAdenaResponseStatus.SUCCESS ||
      response.type === EAdenaResponseType.NETWORK_ALREADY_EXISTS
    ) {
      return;
    }

    throw new Error('unable to add custom network to Adena');
  }
